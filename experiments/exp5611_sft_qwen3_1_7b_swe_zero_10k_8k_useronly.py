...
"""SWE-ZERO 10K @ 8192-token SFT on Qwen3-1.7B-Base — *user-only* variant.
...
Usage:
  uv run iris --config lib/iris/examples/marin.yaml job run \\
      ...
      --job-name exp5611-sft-qwen3-1-7b-10k-useronly \\
      --no-wait \\
...
from experiments.qwen3_chat_template_user_only import (
    QWEN_3_CHAT_TEMPLATE_USER_ONLY,
)
...
DATASET_NAME = "swe_zero_10k_qwen3_2f328e1d_useronly"
...
def create_tokenization_step():
    return default_tokenize(
        ...
        format=ChatLmDatasetFormat(
            chat_template=QWEN_3_CHAT_TEMPLATE_USER_ONLY,
            pack=1,
        ),
    )
...
exp5611_qwen3_sft_useronly = default_sft(
    name=f"exp5611_sft_qwen3_1_7b_swe_zero_10k_{MAX_SEQ_LEN}tokens_arch32k_useronly_{RESOURCE_SUFFIX}",
...
exp5611_qwen3_checkpoint_useronly = exp5611_qwen3_sft_useronly.cd(f"hf/step-{NUM_TRAIN_STEPS - 1}").nonblocking()

if __name__ == "__main__":
    print("=== exp5611 arm (c): Qwen3-1.7B-Base SWE-ZERO 10K user-only SFT ===")
    print(f"Dataset: {PREBUILT_JSONL_GLOB} ({SUBSET_SIZE:,} sampled @ 2f328e1d)")
    print("Chat template: QWEN_3_CHAT_TEMPLATE_USER_ONLY (user-only (task+observations) in loss)")
    ...
    print(f"Checkpoint: {exp5611_qwen3_checkpoint_useronly}")
    executor_main(steps=[exp5611_qwen3_sft_useronly])
