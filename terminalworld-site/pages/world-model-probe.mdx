...
**3. The pure simulator (c) confirms it's the mask, not the data — and the predicted ordering (c) ≤ (b) < (a) holds.** Arm **(c) = user-only loss** (loss on task + observation tokens, the exact complement of (a)) is the apples-to-apples "pure simulator": same agent-format data and same scoring framing as (a)/(b), differing *only* in which tokens get loss. At 10K it lands at **median 1.14 / token-weighted 1.75 — the best world model at its scale**, edging (b) 10K (1.17 / 1.74) and crushing (a) 10K (1.81 / 2.01), with **zero catastrophic trajectories** like (b). Since (a) ⊕ (c) = (b) exactly at the token level, the three arms are a clean decomposition: putting loss on observation tokens is *necessary and sufficient* for the world model — (a) (no observation loss) forgets it, (c) (only observation loss) and (b) (both) build it. The dropped *dedicated* env-rewritten simulator (token-weighted 1.79) wasn't comparable — it was scored in a different framing (observation in the `assistant` role) — and (c) now supersedes it as the rigorous point. (c) at 100K/1M is training to confirm the ordering holds with scale.
